...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .link-grid { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 8px; }
    .tag { display: inline-block; font-size: 11px; color: #888; margin: 8px 0 4px; }
    .input-row { display: flex; gap: 8px; margin: 10px 0 12px; align-items: stretch; }
    .input-row input {
    ...
    .input-row .btn { margin: 0; white-space: nowrap; }
  </style>
</head>
<body>
  ...
  <div class="section">
    <h2>🔗 App Links（https://shoo.app）全类型模拟</h2>
    <p class="info">统一协议：https://shoo.app/... → FlutterBridge deeplink → Resolver(appLink) → Navigator。红色按钮表示需登录。</p>

    <div class="tag">自定义输入（支持 App Link / Deep Link / 相对路径）</div>
    <div class="input-row">
      <input id="customLinkInput" type="url" placeholder="如 https://shoo.app/product/p-1 或 shoo://cart 或 /flash-sale" value="https://shoo.app/product/c1-g1-l1-p1" onkeydown="if(event.key==='Enter'){openCustomLink();}" />
      <button class="btn teal" onclick="openCustomLink()">打开</button>
    </div>
    <div class="link-grid" style="margin-top:0; margin-bottom:8px;">
      <button class="btn" onclick="fillAndOpen('https://shoo.app/profile')">填入 profile</button>
      <button class="btn" onclick="fillAndOpen('shoo://product/c1-g1-l1-p1')">填入 shoo://</button>
      <button class="btn" onclick="fillAndOpen('/orders')">填入 /orders</button>
    </div>

    <div class="tag">基础 / Tab</div>
    <div class="link-grid">
      <button class="btn teal" onclick="openAppLink('https://shoo.app/')">home · 首页</button>
      <button class="btn teal" onclick="openAppLink('https://shoo.app/cart')">cart · 购物车</button>
      <button class="btn teal" onclick="openAppLink('https://shoo.app/profile')">profile · 个人中心</button>
      <button class="btn teal" onclick="openAppLink('https://shoo.app/category')">category · 分类</button>
      <button class="btn teal" onclick="openAppLink('https://www.shoo.app/profile')">www.shoo.app · profile</button>
    </div>

    <div class="tag">商品</div>
    <div class="link-grid">
      <button class="btn" onclick="openAppLink('https://shoo.app/product/c1-g1-l1-p1')">productDetail · 商品详情</button>
      <button class="btn" onclick="openAppLink('https://shoo.app/product/c1-g1-l1-p1/reviews')">productDetail · 商品评价</button>
      <button class="btn" onclick="openAppLink('https://shoo.app/category/products?leafId=c1-g1-l1&amp;title=%E8%B0%83%E8%AF%95%E5%95%86%E5%93%81%E5%88%97%E8%A1%A8')">productList · 商品列表</button>
    </div>

    <div class="tag">搜索 / 营销</div>
    <div class="link-grid">
      <button class="btn green" onclick="openAppLink('https://shoo.app/search')">search · 搜索</button>
      <button class="btn green" onclick="openAppLink('https://shoo.app/search?q=shoes')">search · 搜索带参</button>
      <button class="btn green" onclick="openAppLink('https://shoo.app/flash-sale')">flashSale · 抢购</button>
      <button class="btn green" onclick="openAppLink('https://shoo.app/flash-sale?activityId=act-demo')">flashSale · 指定活动</button>
      <button class="btn green" onclick="openAppLink('https://shoo.app/new-arrivals')">newArrivals · 新品</button>
      <button class="btn green" onclick="openAppLink('https://shoo.app/trending')">trending · 热门</button>
      <button class="btn green" onclick="openAppLink('https://shoo.app/activity')">activity · 活动页</button>
    </div>

    <div class="tag">账号</div>
    <div class="link-grid">
      <button class="btn orange" onclick="openAppLink('https://shoo.app/login')">login · 登录</button>
      <button class="btn orange" onclick="openAppLink('https://shoo.app/login?redirect=%2Fcheckout')">login · 登录后回 checkout</button>
    </div>

    <div class="tag">交易（需登录）</div>
    <div class="link-grid">
      <button class="btn auth" onclick="openAppLink('https://shoo.app/orders')">orders · 订单列表</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/orders/o-demo-1')">orderDetail · 订单详情</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/orders/o-demo-1/logistics')">orderDetail · 物流</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/payment/o-demo-1')">payment · 支付</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/checkout')">checkout · 结算</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/coupons')">coupons · 优惠券</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/coupons?select=1')">coupons · 选券</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/after-sales')">afterSales · 售后</button>
      <button class="btn auth" onclick="openAppLink('https://shoo.app/after-sales/apply/o-demo-1')">afterSales · 申请售后</button>
    </div>

    <div class="tag">WebView</div>
    <div class="link-grid">
      <button class="btn purple" onclick="openAppLink('https://shoo.app/webview?url=https%3A%2F%2Fflutter.dev&amp;title=Flutter')">webview · 打开 Flutter 官网</button>
    </div>
  </div>
  ...
  <script>
    ...
    function openAppLink(url) {
      // App Links 专用入口：显式标记 link_kind，便于 Flutter 侧日志区分
      try {
        if (window.FlutterBridge) {
    ...
    function openCustomLink() {
      ...
      if (!raw) {
        alert('请输入 Deep Link 或 App Link');
        return;
      }
      if (raw.indexOf('https://shoo.app') === 0 || raw.indexOf('http://shoo.app') === 0 ||
          raw.indexOf('https://www.shoo.app') === 0 || raw.indexOf('http://www.shoo.app') === 0) {
        openAppLink(raw);
      } else {
        openDeepLink(raw);
      }
