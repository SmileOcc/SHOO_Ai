...
import 'package:shoo/features/toolbox/domain/entities/hos_download_preview_support.dart';
...
import 'package:shoo/features/toolbox/data/datasources/remote/hos_txt_chapter_index_cache.dart';
import 'package:shoo/features/toolbox/presentation/widgets/hos_txt_reader_pagination.dart';
import 'package:shoo/features/toolbox/presentation/widgets/hos_txt_reader_session.dart';
import 'package:shoo/features/toolbox/presentation/widgets/hos_txt_reader_theme.dart';
...
class _SHOTxtReaderPageState extends ConsumerState<SHOTxtReaderPage>
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      final chapterIndex = clampListIndex(
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (session.flatPages.isEmpty) return;
        final idx = clampListIndex(
          session.currentFlatIndex,
          session.flatPages.length,
        );
        session.currentFlatIndex = idx;
  ...
  void _syncPageController(int index) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final session = _session;
      if (session == null || session.flatPages.isEmpty) return;
      final clamped = clampListIndex(index, session.flatPages.length);
      session.currentFlatIndex = clamped;
      _pageController.jumpToPage(clamped);
    });
  }

  void _jumpPageControllerToSessionIndex() {
    final session = _session;
    if (session == null || !_pageController.hasClients) return;
    if (session.flatPages.isEmpty) return;
    final idx = clampListIndex(
      session.currentFlatIndex,
      session.flatPages.length,
  ...
  void _handlePageChanged(int index) {
    final session = _session;
    if (session == null || session.flatPages.isEmpty) return;
    final safeIndex = clampListIndex(index, session.flatPages.length);
    _markUserNavigated();
    setState(() => session.currentFlatIndex = safeIndex);
    _setShowChrome(false);
    unawaited(_handlePageChangedAsync(safeIndex));
  }

  Future<void> _handlePageChangedAsync(int index) async {
    final session = _session;
    if (session == null || !mounted || session.flatPages.isEmpty) return;
    final safeIndex = clampListIndex(index, session.flatPages.length);
    ...
    final page = session.flatPages[safeIndex];
  ...
  Future<void> _repaginate({double? fontSize}) async {
    ...
    await _persistProgress();
    _jumpPageControllerToSessionIndex();
  }
  ...
  String _bottomChromeChapterTitle(
    ...
    if (_sliderPreviewChapterIndex != null) {
      final idx = clampListIndex(
        _sliderPreviewChapterIndex!,
        session.chapterMetas.length,
      );
