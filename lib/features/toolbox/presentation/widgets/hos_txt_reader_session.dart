...
class SHOTxtReaderSession {
  ...
  SHONovelPage? get currentPage {
    ...
    final idx = clampListIndex(currentFlatIndex, flatPages.length);
  ...
  Future<List<SHONovelPage>> loadChapter({
    ...
    if (prepend) {
      ...
      final target = clampListIndex(jumpToPageInChapter ?? 0, pages.length);
  ...
  Future<void> repaginateAllLoaded({
    ...
    if (loaded.isEmpty && chapterMetas.isNotEmpty) {
      loaded.add(clampListIndex(chapter, chapterMetas.length));
    }

    final rebuilt = SHOTxtReaderSession(file: file, chapterMetas: chapterMetas);
    for (final index in loaded) {
      await rebuilt.loadChapter(
        chapterIndex: index,
        pagination: pagination,
        append: rebuilt.flatPages.isNotEmpty,
      );
    }
    if (rebuilt.flatPages.isEmpty && chapterMetas.isNotEmpty) {
      await rebuilt.loadChapter(
        chapterIndex: clampListIndex(chapter, chapterMetas.length),
        pagination: pagination,
      );
    }
    await rebuilt.jumpToChapter(
      chapterIndex: clampListIndex(chapter, chapterMetas.length),
      pagination: pagination,
    ...
    flatPages
      ..clear()
      ..addAll(rebuilt.flatPages);
    loadedChapters
      ..clear()
      ..addAll(rebuilt.loadedChapters);
    currentFlatIndex = clampListIndex(
      rebuilt.currentFlatIndex,
      flatPages.length,
    );
  }
}
